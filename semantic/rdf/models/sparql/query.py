"""
Create SPARQL statements for QuerySets.

The code in here encapsulates all of the SPARQL construction so that QuerySets
themselves do not have to (and could be backed by things other than SPARQL
...
class RawSemanticQuery(object):
    """
    A single raw SPARQL query
    """
    ...
    def clone(self, using):
        return RawQuery(self.sparql, using, params=self.params)
    ...
    def __repr__(self):
        return "<RawQuery: %r>" % (self.sparql % self.params)

    def _execute_query(self):
        self.cursor = connections[self.using].cursor()
        self.cursor.execute(self.sparql, self.params)
...
class SparqlQuery(Query):
    ...
    """
    # SPARQL join types. These are part of the class because their string forms
    ...
    def __init__(self, model, where=WhereNode):
        ...
        # SPARQL aggregate-related attributes
        self.aggregates = SortedDict() # Maps alias -> SPARQL aggregate function
        ...
        self.extra = SortedDict()  # Maps col_alias -> (col_sparql, params).
    ...
    def __str__(self):
        """
        Returns the query as a string of SPARQL with the parameter values
        substituted in.
    ...
    def get_aggregation(self, using):
        ...
        result = query.get_compiler(using).execute_sparql(SINGLE)
        if result is None:
    ...
    def get_count(self, using):
        ...
        # in SPARQL (in variants that provide them) doesn't change the COUNT
        # output.
    ...
    def has_results(self, using):
        ...
        return bool(compiler.execute_sparql(SINGLE))
    ...
    def join(self, connection, always_create=False, exclusions=(),
        ...
        table alias or a table name. The join correspods to the SPARQL equivalent
        of::
    ...
    def setup_inherited_models(self):
        ...
        all the select columns (this method is called from pre_sparql_setup(),
    ...
    def add_filter(self, filter_expr, connector=AND, negate=False, trim=False,
        ...
        # Interpret '__exact=None' as the sparql 'is NULL'; otherwise, reject all
        ...
        if value is None:
            ...
            value = SPARQLEvaluator(value, self)
        ...
        if negate:
            ...
            if lookup_type != 'isnull':
                ...
                if not (lookup_type == 'in'
                            and not hasattr(value, 'as_sparql')
                            and not hasattr(value, '_as_sparql')
    ...
    def set_limits(self, low=None, high=None):
        ...
        as it makes it more Pythonic to read and write. When the SPARQL query is
    ...
    def add_count_column(self):
        ...
        if not self.distinct:
            ...
            if not self.select:
                ...
                # Because of SPARQL portability issues, multi-column, distinct
    ...
    def add_deferred_loading(self, field_names):
        ...
        # splitting and handling when computing the SPARQL colum names (as part of
        # get_columns()).
    ...
    def add_immediate_loading(self, field_names):
        """
        Add the given list of model field names to the set of fields to
        retrieve when the SPARQL is executed ("immediate loading" fields). The
