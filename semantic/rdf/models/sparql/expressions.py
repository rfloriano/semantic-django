...
class SPARQLEvaluator(object):
    ...
    def evaluate_node(self, node, qn, connection):
        ...
        for child in node.children:
            if hasattr(child, 'evaluate'):
                sparql, params = child.evaluate(self, qn, connection)
            else:
                sparql, params = '%s', (child,)
            ...
            if sparql:
                expressions.append(format % sparql)
    ...
    def evaluate_leaf(self, node, qn, connection):
        col = self.cols[node]
        if hasattr(col, 'as_sparql'):
            return col.as_sparql(qn, connection), ()
        else:
            return '%s.%s' % (qn(col[0]), qn(col[1])), ()

    def evaluate_date_modifier_node(self, node, qn, connection):
        timedelta = node.children.pop()
        sparql, params = self.evaluate_node(node, qn, connection)

        if timedelta.days == 0 and timedelta.seconds == 0 and \
                timedelta.microseconds == 0:
            return sparql, params

        return connection.ops.date_interval_sparql(sparql, node.connector, timedelta), params
