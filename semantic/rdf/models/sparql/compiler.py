...
class SPARQLCompiler(object):
    ...
    def pre_sparql_setup(self):
        """
        Does any necessary class setup immediately prior to producing SPARQL. This
    ...
    def quote_name_unless_alias(self, name):
        ...
        for table names. This avoids problems with some SPARQL dialects that treat
        quoted strings specially (e.g. PostgreSPARQL).
        """
...
# """, []
    ...
    def as_nested_sparql(self):
        """
        Perform the same functionality as the as_sparql() method, returning an
        SPARQL string and parameters. However, the alias prefixes are bumped
    ...
    def get_columns(self, with_aliases=False):
        ...
        if self.query.select:
            ...
            for col in self.query.select:
                if isinstance(col, (list, tuple)):
                    ...
                    result.append(col.as_sparql(qn, self.connection))
        ...
        result.extend([
            '%s%s' % (
                aggregate.as_sparql(qn, self.connection),
                alias is not None
    ...
    def get_default_columns(self, with_aliases=False, col_aliases=None,
        ...
        Returns a list of strings, quoted appropriately for use in SPARQL
        ...
        for field, model in opts.get_fields_with_model():
            ...
            if start_alias:
                ...
                # aliases will have already been set up in pre_sparql_setup(), so
    ...
    def get_ordering(self):
        """
        Returns a tuple containing a list representing the SPARQL elements in the
        "order by" clause, and the list of SPARQL elements that need to be added
        to the GROUP BY clause as a result of the ordering.

        Also sets the ordering_aliases attribute on this instance to a list of
        extra aliases needed in the select.

        Determining the ordering SPARQL can change the tables we need to include,
        ...
        for field in ordering:
            if field == '?':
                result.append(self.connection.ops.random_function_sparql())
                continue
    ...
    def get_from_clause(self):
        ...
        This should only be called after any SPARQL construction methods that
    ...
    def get_grouping(self):
        """
        Returns a tuple representing the SPARQL elements in the "group by" clause.
        """
        qn = self.quote_name_unless_alias
        result, params = [], []
...
class SPARQLInsertCompiler(SPARQLCompiler):
    ...
    def execute_sparql(self, return_id=False):
        ...
        cursor = super(SPARQLInsertCompiler, self).execute_sparql(None)
...
class SPARQLDeleteCompiler(SPARQLCompiler):
    def as_sparql(self):
        """
        Creates the SPARQL for this query. Returns the SPARQL string and list of
        parameters.
...
class SPARQLUpdateCompiler(SPARQLCompiler):
    def as_sparql(self):
        """
        Creates the SPARQL for this query. Returns the SPARQL string and list of
        parameters.
    ...
    def execute_sparql(self, result_type):
        ...
        """
        cursor = super(SPARQLUpdateCompiler, self).execute_sparql(result_type)
        rows = cursor and cursor.rowcount or 0
        is_empty = cursor is None
        del cursor
        for query in self.query.get_related_updates():
            aux_rows = query.get_compiler(self.using).execute_sparql(result_type)
            if is_empty:
                rows = aux_rows
                is_empty = False
        return rows
    ...
    def pre_sparql_setup(self):
        ...
        (portable) SPARQL updates. That is done here.
        ...
        super(SPARQLUpdateCompiler, self).pre_sparql_setup()
        ...
        if self.query.related_updates or must_pre_select:
            ...
            # selecting from the updating table (e.g. MySPARQL).
            idents = []
            for rows in query.get_compiler(self.using).execute_sparql(MULTI):
...
class SPARQLAggregateCompiler(SPARQLCompiler):
    def as_sparql(self, qn=None):
        """
        Creates the SPARQL for this query. Returns the SPARQL string and list of
        parameters.
        """
        if qn is None:
            qn = self.quote_name_unless_alias
        sparql = ('SELECT %s FROM (%s) subquery' % (
            ', '.join([
                aggregate.as_sparql(qn, self.connection)
...
class SPARQLDateCompiler(SPARQLCompiler):
    def results_iter(self):
        """
        Returns an iterator over the results from executing this query.
        """
...
def order_modified_iter(cursor, trim, sentinel):
    ...
    the results, since they're only needed to make the SPARQL valid.
    """
